...
namespace ServiceHelpers
{
    ...
    {
        private static FaceAttributeType[] DefaultFaceAttributeTypes = new FaceAttributeType[] { FaceAttributeType.Age, FaceAttributeType.Gender, FaceAttributeType.Emotion };
        ...
        public IEnumerable<Microsoft.ProjectOxford.Common.Contract.Emotion> DetectedEmotion { get; set; }
        ...
        public async Task DetectFacesAsync(bool detectFaceAttributes = true, bool detectFaceLandmarks = false)
        {
            try
            {
        ...
        public async Task DetectEmotionAsync()
        {
            ...
            catch (Exception e)
            {
                ...
                this.DetectedEmotion = Enumerable.Empty<Microsoft.ProjectOxford.Common.Contract.Emotion>();
        ...
        public async Task IdentifyFacesByGroupAsync(string personGroupId)
        {
            this.IdentifiedPersons = Enumerable.Empty<IdentifiedPerson>();
            ...
            if (detectedFaceIds != null && detectedFaceIds.Any())
            {
                ...
                try
                {
                    IdentifyResult[] groupResults = await FaceServiceHelper.IdentifyAsync(personGroupId, detectedFaceIds);
                    foreach (var match in groupResults)
                    {
                        if (!match.Candidates.Any())
                        {
                        ...
                        Person person = await FaceServiceHelper.GetPersonAsync(personGroupId, match.Candidates[0].PersonId);
